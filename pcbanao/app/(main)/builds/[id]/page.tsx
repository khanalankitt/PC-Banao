...
const CAT_META: Record<string, CatMeta> = {
  ...
  motherboard: {
    accent: '#fbbf24', label: 'Motherboard',
    Icon: () => (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path d="M5 14h4M5 17h4" strokeWidth="1" />
      </svg>
    ),
  },
  ram: {
    accent: '#34d399', label: 'Memory',
    Icon: () => (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  psu: {
    accent: '#fb923c', label: 'Power Supply',
    Icon: () => (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (user.image) {
    return (
      ...
      <img src={user.image} alt={user.name}
        style={{ width: size, height: size, borderRadius: '50%', border: '1.5px solid rgba(255,255,255,0.12)', flexShrink: 0, objectFit: 'cover' }}
      />
    );
  }
  return (
...
export default function ViewBuildPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/api/builds/${id}`)
      .then((res) => {
        if (res.data?.success && res.data?.data) setBuild(normaliseApiBuild(res.data.data));
  ...
  function handleClone() {
    if (!build) return;
    loadIntoStore({ ...build, name: `${build.name} (Clone)` }, false);
    router.push('/builder');
  }

  function handleEdit() {
    if (!build) return;
    loadIntoStore(build, true);
    router.push('/builder');
  }

  async function handleDelete() {
    setDeleting(true);
    try { await api.delete(`/api/builds/${id}`); router.push('/builds'); }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--bg-void)' }}>
        <Navbar />
        <div style={{ maxWidth: '860px', margin: '0 auto', padding: '120px 24px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[100, 60, 60, 60, 60].map((h, i) => (
              <div key={i} style={{ height: h, borderRadius: '10px', background: 'rgba(255,255,255,0.025)', animation: `pulseGlow 1.5s ease-in-out ${i * 90}ms infinite` }} />
            ))}
          </div>
        </div>
      </div>
  ...
  if (notFound || !build) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--bg-void)' }}>
        <Navbar />
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 56px)', marginTop: '56px', textAlign: 'center', padding: '40px 24px', gap: '12px' }}>
          <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '4px' }}>
          ...
          <h1 style={{ fontSize: '20px', fontWeight: 800, color: '#f0f8ff', margin: 0 }}>Build not found</h1>
          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '13px', margin: 0 }}>
            This build may have been deleted or set to private.
          </p>
          <Link href="/builds" style={{ marginTop: '8px', padding: '9px 20px', borderRadius: '8px', background: 'linear-gradient(135deg, #22d3ee, #38bdf8)', color: '#020408', fontWeight: 800, fontSize: '12px', textDecoration: 'none', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            Back to builds
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-void)', color: '#f0f8ff' }}>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '88px 24px 100px' }}>
        ...
        <Link
          href="/builds"
          style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', color: 'rgba(255,255,255,0.3)', fontSize: '12px', textDecoration: 'none', marginBottom: '28px', transition: 'color 0.15s' }}
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: compatible
              ? 'linear-gradient(90deg, #34d399, #34d39940 60%, transparent)'
              : 'linear-gradient(90deg, #f87171, #f8717140 60%, transparent)',
          }} />
          ...
          <div style={{ padding: '28px 30px 24px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', marginBottom: '18px', flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <h1 style={{ margin: '0 0 10px', fontSize: 'clamp(22px, 4vw, 32px)', fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1.1, color: '#f0f8ff' }}>
                  {build.name}
                </h1>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  ...
                  <span style={{
                    ...
                    background: build.isPublic ? 'rgba(34,211,238,0.08)' : 'rgba(255,255,255,0.04)',
                    border: `1px solid ${build.isPublic ? 'rgba(34,211,238,0.2)' : 'rgba(255,255,255,0.08)'}`,
                    color: build.isPublic ? '#22d3ee' : 'rgba(255,255,255,0.3)',
                  }}>
              ...
              <div style={{
                ...
                background: compatible ? 'rgba(52,211,153,0.08)' : 'rgba(248,113,113,0.08)',
                border: `1px solid ${compatible ? 'rgba(52,211,153,0.2)' : 'rgba(248,113,113,0.2)'}`,
              }}>
                ...
                <span style={{ fontSize: '12px', fontWeight: 700, color: compatible ? '#34d399' : '#f87171', letterSpacing: '0.04em' }}>
                  {compatible ? 'Compatible' : 'Has issues'}
                </span>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '22px' }}>
              ...
              <div style={{ padding: '14px 16px', borderRadius: '10px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                ...
                <div style={{ marginTop: '6px', height: '2px', background: 'rgba(255,255,255,0.06)', borderRadius: '1px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${wPct}%`, background: wColor, borderRadius: '1px', transition: 'width 0.6s ease' }} />
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <button
                ...
                style={{
                  ...
                  boxShadow: '0 4px 16px rgba(34,211,238,0.2)', transition: 'opacity 0.15s',
                }}
              ...
              {isOwner && (
                <>
                  <button
                    onClick={handleEdit}
                    style={{ padding: '10px 18px', borderRadius: '8px', background: 'rgba(167,139,250,0.09)', border: '1px solid rgba(167,139,250,0.22)', color: '#a78bfa', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', cursor: 'pointer', transition: 'opacity 0.15s' }}
                  ...
                  <button
                    ...
                    style={{ padding: '10px 18px', borderRadius: '8px', background: 'rgba(248,113,113,0.07)', border: '1px solid rgba(248,113,113,0.18)', color: '#f87171', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', cursor: 'pointer', transition: 'opacity 0.15s' }}
                    onMouseEnter={(e) => (e.currentTarget.style.opacity = '0.7')}
                    onMouseLeave={(e) => (e.currentTarget.style.opacity = '1')}
                  >
                    Delete
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.25)' }}>
              Part List
            </span>
            <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.2)' }}>
              {build.components.length} items
            </span>
          </div>
            ...
            return (
              ...
              >
                ...
                <div style={{
                  ...
                  border: `1px solid ${meta.accent}1e`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  color: meta.accent,
                }}>
                  <meta.Icon />
                ...
                <div style={{ width: '90px', flexShrink: 0 }}>
                  <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: meta.accent, opacity: 0.75 }}>
                    {meta.label}
                  </span>
                </div>
                ...
                {/* Part info */}
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: '#f0f8ff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '2px' }}>
                    {c.part.name}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    {c.part.wattage && (
                      <>
                        ...
                        <span style={{ fontSize: '11px', color: '#fb923c' }}>{c.part.wattage}W</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div style={{ flexShrink: 0, textAlign: 'right' }}>
                  <span style={{ fontSize: '15px', fontWeight: 800, color: '#f0f8ff', letterSpacing: '-0.02em' }}>
                    ${c.part.price.toLocaleString()}
                  </span>
                </div>
              </div>
            );
          ...
          <div style={{
            ...
            <span style={{ fontSize: '20px', fontWeight: 900, color: '#f0f8ff', letterSpacing: '-0.03em' }}>
      ...
      {confirmDelete && (
        <div
          ...
          style={{
            position: 'fixed', inset: 0, background: 'rgba(2,4,8,0.9)', backdropFilter: 'blur(14px)',
            display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '24px',
          }}
        >
          <div style={{
            background: '#090e18',
            border: '1px solid rgba(248,113,113,0.2)', borderRadius: '14px',
            padding: '28px', width: '100%', maxWidth: '370px', textAlign: 'center',
            boxShadow: '0 32px 80px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.04)',
          }}>
            <div style={{
              width: '44px', height: '44px', borderRadius: '10px', margin: '0 auto 16px',
            ...
            <p style={{ margin: '0 0 24px', fontSize: '13px', color: 'rgba(255,255,255,0.4)', lineHeight: 1.65 }}>
              <strong style={{ color: 'rgba(255,255,255,0.75)', fontWeight: 600 }}>{build.name}</strong> will be permanently removed and cannot be recovered.
            </p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button
                onClick={() => !deleting && setConfirmDelete(false)}
                style={{ flex: 1, padding: '10px', borderRadius: '8px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.5)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', cursor: deleting ? 'not-allowed' : 'pointer', textTransform: 'uppercase' }}
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                style={{ flex: 1, padding: '10px', borderRadius: '8px', background: deleting ? 'rgba(248,113,113,0.5)' : '#ef4444', border: 'none', color: '#fff', fontSize: '12px', fontWeight: 800, letterSpacing: '0.06em', textTransform: 'uppercase', cursor: deleting ? 'not-allowed' : 'pointer' }}
              >
